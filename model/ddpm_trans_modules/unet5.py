...
from model.ddpm_trans_modules.trans_block_eca2 import TransformerBlock_eca
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv1_t = nn.Sequential(
            nn.Conv2d(6, dim, kernel_size=3, stride=1, padding=1, bias=False))
        # ,nn.PixelUnshuffle(2))
        self.conv2_t = nn.Sequential(
            nn.Conv2d(6, dim // 2, kernel_size=3, stride=1, padding=1),
            nn.PixelUnshuffle(2))
        self.conv3_t = nn.Sequential(
            nn.Conv2d(6, int(dim * 2 ** 1) // 8, kernel_size=3, stride=1, padding=1),
            nn.PixelUnshuffle(2), nn.PixelUnshuffle(2))
        self.conv4_t = nn.Sequential(
            nn.Conv2d(6, int(dim * 2 ** 2) // 32, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, x, t,df):
        # 1
        x = self.conv1(x)
        p_t1 = self.conv1_t(df)
        x1 = self.block1(x, t,  p_t1)
        x1 = self.block1(x1, t,  p_t1)  #注释
        x1 = self.block1(x1, t,  p_t1)

        # 2
        x2 = self.conv2(x1)
        p_t2 = self.conv2_t(df)
        x2 = self.block2(x2, t, p_t2)
        x2 = self.block2(x2, t, p_t2) #注释
        x2 = self.block2(x2, t, p_t2)

        # 3
        x3 = self.conv3(x2)
        p_t3 = self.conv3_t(df)
        x3 = self.block3(x3, t, p_t3)
        x3 = self.block3(x3, t, p_t3) #注释
        x3 = self.block3(x3, t, p_t3)

        # 4
        x4 = self.conv4(x3)
        p_t4 = self.conv4_t(df)
        x4 = self.block4(x4, t, p_t4)

        de_level3 = self.conv_up3(x4)
        de_level3 = torch.cat([de_level3, x3], 1)
        de_level3 = self.conv_cat3(de_level3)
        de_level3 = self.decoder_block3(de_level3, t, p_t3)
        de_level3 = self.decoder_block3(de_level3, t, p_t3) #注释
        de_level3 = self.decoder_block3(de_level3, t, p_t3)

        de_level2 = self.conv_up2(de_level3)
        de_level2 = torch.cat([de_level2, x2], 1)
        de_level2 = self.conv_cat2(de_level2)
        de_level2 = self.decoder_block2(de_level2, t, p_t2)
        de_level2 = self.decoder_block2(de_level2, t, p_t2) #注释
        de_level2 = self.decoder_block2(de_level2, t, p_t2)

        de_level1 = self.conv_up1(de_level2)
        de_level1 = torch.cat([de_level1, x1], 1)
        de_level1 = self.conv_cat1(de_level1)
        mid_feat = self.decoder_block1(de_level1, t, p_t1)
        mid_feat = self.decoder_block1(mid_feat, t, p_t1) #注释
        mid_feat = self.decoder_block1(mid_feat, t, p_t1)
...
class UNet(nn.Module):
    ...
    def forward(self, x, time ,df):
        t = self.time_mlp(time) if exists(self.time_mlp) else None

        mid_feat = self.encoder_water(x, t ,df)
...
if __name__ == '__main__':
    ...
    img = torch.zeros(2, 6, 256, 256)
    img_t = torch.zeros(2, 3, 256, 256)
    time = torch.tensor([1, 2])
    model = UNet(inner_channel=48, norm_groups=24, in_channel=6)
    a=model(img, time, img_t)
    print(a.shape)
