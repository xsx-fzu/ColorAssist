...
class DDPM(BaseModel):
    ...
    def optimize_parameters2(self):
        ...
        l_pix = self.netG(self.data)
        # need to average in multi-gpu
        b, c, h, w = self.data['original'].shape
    ...
    def test(self, cand=None, continous=False):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.recolor = self.netG.module.super_resolution(
                    self.data, continous)
            else:
                # n = None
                # self.temp, miu, var = self.netG_air.super_resolution(
                #     self.data, continous, flag='style', n=n)
                self.recolor = self.netG.super_resolution(
                    self.data, continous, cand=None)
                # self.recolor = self.netG.super_resolution(
                #     self.data, continous)
    ...
    def sample(self, batch_size=1, continous=False):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.recolor = self.netG.module.sample(batch_size, continous)
            else:
                self.recolor = self.netG.sample(batch_size, continous)
        self.netG.train()
    ...
    def get_current_visuals(self, sample=False):
        ...
        if sample:
            out_dict['SAM'] = self.recolor.detach().float().cpu()
        else:
            out_dict['recolor'] = self.recolor.detach().float().cpu()
            out_dict['inf'] = self.data['inf'].detach().float().cpu()
            out_dict['original'] = self.data['original'].detach().float().cpu()
        return out_dict
