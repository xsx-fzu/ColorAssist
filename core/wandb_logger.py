...
class WandbLogger:
    ...
    def __init__(self, opt):
        ...
        if self.config.get('log_eval', None):
            self.eval_table = self._wandb.Table(columns=['gen_image',
                                                         'recolor_image',
                                                         'original_image',
                                                         'psnr',
                                                         'ssim'])
        ...
        if self.config.get('log_infer', None):
            self.infer_table = self._wandb.Table(columns=['gen_image',
                                                         'recolor_image',
                                                         'original_image'])
        else:
    ...
    def log_eval_data(self, gen_img, recolor_img, original_img, psnr=None, ssim=None):
        """
        Add data row-wise to the initialized table.
        """
        if psnr is not None and ssim is not None:
            self.eval_table.add_data(
                self._wandb.Image(gen_img),
                self._wandb.Image(recolor_img),
                self._wandb.Image(original_img),
                psnr,
                ssim
            )
        else:
